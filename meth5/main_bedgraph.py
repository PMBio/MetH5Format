...
def set_arguments(sc_args: argparse.ArgumentParser):
    ...
    sc_args.add_argument("-r", "--region", type=argtype_genomic_range, required=True,
        help="Genomic region formatted as chr:start-end", )
    
def main(input_m5_file: Path, region: Dict, chunk_size: int):
    with MetH5File(input_m5_file, "r", chunk_size=chunk_size) as f:
        ...
        print("track type=bedGraph name=methylation description=center_label visibility=display_mode color=252,127,44 altColor=25,4,248 graphType=heatmap viewLimits=0:1 midRange=0.50:0.50 midColor=255,255,255")
