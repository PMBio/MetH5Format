...
def main():
    ...
    sc_args = subparsers.add_parser("merge_m5", description="Merge m5 file from Nanopolish result files", )
    sc_args.set_defaults(func=meth5.main_merge_m5.main)

    sc_args.add_argument("--input_m5_files", type=Path, required=True, nargs="+",
        help="List of MetH5 files", )
    
    sc_args.add_argument("--read_group_names", type=str, required=True, nargs="+",
        help="One name per input file", )

    sc_args.add_argument("--output_file", type=Path, required=True, help="Output MetH5 file", )

    sc_args.add_argument("--quiet", action="store_true", help="No progress bar or warnings will be displayed", )

    sc_args.add_argument("--compression", type=str, required=False, default="gzip", choices=["gzip", "None"],
        help="Compression method for the MetH5 data structures. Use 'gzip' for smaller file size, or 'None' for "
             "faster read and write speeds", )

    sc_args.add_argument("--allowed_chromosomes", type=str, nargs="+", required=False, default=None,
        help="Only include these chromosomes", )
